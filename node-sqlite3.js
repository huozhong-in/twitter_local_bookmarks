...
import { argv as _argv, env, platform as _platform } from 'process';

const argv = key => {
    // Return true if the key exists and a value is defined
    if ( _argv.includes( `--${ key }` ) ) return true;
    const value = _argv.find( element => element.startsWith( `--${ key }=` ) );
...
db.serialize(function() {
    ...
    // load extension
    var platform = env.npm_config_target_platform || _platform
    if (platform === 'win32') {
      db.loadExtension(join(ext_path, "simple"));
    } else {
      db.loadExtension(join(ext_path, "libsimple"));
    }
